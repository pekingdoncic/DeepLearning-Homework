...
for epoch in range(settings.EPOCHS):
    ...
    with tqdm(total=settings.STEPS_PER_EPOCH, desc='Epoch {}/{}'.format(epoch + 1, settings.EPOCHS)) as pbar:
        ...
        for step in range(settings.STEPS_PER_EPOCH):
            ...
            pbar.update(1)
        # 每个epoch保存一次图片
        generated_image = noise_image.numpy()
        generated_image = generated_image.squeeze()
        plt.imshow(generated_image)
        plt.axis('off')
        plt.show()
        # 每个epoch保存一次图片
        utils.save_image(noise_image, '{}/style_transfer_{}-{}.jpg'.format(settings.OUTPUT_DIR, epoch + 1, settings.EPOCHS))
        #绘制损失折线图
        losses.append(_loss)  # 假设在代码的开始有 losses = [] 这一行
